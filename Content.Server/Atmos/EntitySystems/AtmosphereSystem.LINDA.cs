...
namespace Content.Server.Atmos.EntitySystems
{
    public sealed partial class AtmosphereSystem
    {
        ...
        public float Share(TileAtmosphere tileReceiver, TileAtmosphere tileSharer, int atmosAdjacentTurfs)
        {
            ...
            // --- НЕ смешиваем космическую воду с соседями ---
            // Космическая вода (Immutable) не должна смешиваться
            if (receiver.Immutable && receiver.GetMoles(Gas.Water) > 0)
                return 0f;

            if (sharer.Immutable && sharer.GetMoles(Gas.Water) > 0)
                return 0f;
